 ...
 *
 * A suggestion with `suggestedStartAt` set needs a different time than the one
 * originally asked for. That is stated on the row and confirmed again on the
 * next screen — never applied quietly.
 */
export function QuickBookSuggestions({
  ...
  const allNeedNewTime = suggestions.every(
    (suggestion) => suggestion.suggestedStartAt != null,
  );
      ...
      <ul className="mt-2.5 space-y-2">
        {suggestions.map((suggestion) => (
          <li key={suggestion.providerServiceId}>
            <Link
              href={replacementHref(bookingId, suggestion)}
...
/**
 * Deep-link into the replacement flow with this student preselected. `at` is
 * carried for a time-shifted pick and re-validated server-side against the
 * database's own suggestions — the query string is never trusted.
 */
function replacementHref(
  bookingId: string,
