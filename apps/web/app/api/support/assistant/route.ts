...
import { buildSupportInstructions, buildSupportPageContext, hashSafetyIdentifier, isAiSupportEnabled, safeActionsForContext, SUPPORT_MODEL, SUPPORT_PROMPT_VERSION } from "@/lib/support/assistant";
...
export async function POST(request: Request) {
  ...
  const context = await buildSupportPageContext(supabase, user.id, parsed.data.sourcePath);
    ...
    async start(controller) {
      ...
      try {
        ...
        for await (const event of response) {
          ...
          if (event.type === "response.refusal.delta") send({ type: "delta", text: event.delta });
          ...
          if (event.type === "error" || event.type === "response.failed") throw new Error("upstream_failed");
        }
        completed = true;
        send({ type: "done" });
        ...
        if (!clientAborted) send({ type: "error", code: timedOut ? "timeout" : "unavailable" });
      } finally {
