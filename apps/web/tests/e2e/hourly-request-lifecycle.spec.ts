  ...
  const { error: providerError } = await admin.from("provider_profiles").insert([
    {
      ...
      avatar_image_path: `${providerOneUserId}/e2e-avatar.jpg`,
    ...
    },
    {
      ...
      avatar_image_path: `${providerTwoUserId}/e2e-avatar.jpg`,
      verification_status: "approved",
...
test.afterAll(async () => {
  await runTeardown([
    ...
    { label: "auth.users provider one", run: () => admin.auth.admin.deleteUser(providerOneUserId) },
    { label: "auth.users provider two", run: () => admin.auth.admin.deleteUser(providerTwoUserId) },
  ]);
});
